...
fn spawn_task_or_modal(workspace: &mut Workspace, action: &Spawn, cx: &mut ViewContext<Workspace>) {
    match action {
        Spawn::ByName { task_name } => {
            spawn_task_with_name(task_name.clone(), cx).detach_and_log_err(cx)
        }
        Spawn::ViaModal { target } => toggle_modal(workspace, *target, cx).detach(),
    }
}
...
fn spawn_task_with_name(
    ...
    cx.spawn(|workspace, mut cx| async move {
        ...
        if !did_spawn {
            workspace
                .update(&mut cx, |workspace, cx| {
                    spawn_task_or_modal(workspace, &Spawn::modal(), cx);
                })
                .ok();
        }
